  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (data.user.role !== role) {
        setSubmitErr(`This account is registered as a ${data.user.role}, not a ${role}.`);
        return;
      }
      setToken(data.token);
    ...
    <div style={bg}>
      <div style={card}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          ...
          }}>{isDoctor ? 'Doctor Login' : 'Patient Login'}</h1>
        ...
        <p style={{ textAlign: 'center', marginTop: '1.5rem', fontSize: '0.82rem', color: 'rgba(255,255,255,0.35)' }}>
          ...
          <Link to={`/${role}/register`} style={{ color: '#14b8a6', fontWeight: 600, textDecoration: 'none' }}>Register</Link>
        </p>
